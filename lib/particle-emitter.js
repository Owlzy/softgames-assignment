 ...
 * Compiled Thu, 24 Apr 2025 12:30:10 UTC
 *
...
(function (exports, pixi_js) {
    ...
    /**
     * A Container implementation that manages children in a doubly-linked list
     * for optimized add/remove without array splices.
     */
    class LinkedListContainer extends pixi_js.Container {
        constructor() {
            super(...arguments);
        ...
        /** Override to maintain linked list ordering */
        addChild(...children) {
            ...
            // If already a child, move to end
            if (child.parent === this) {
                this._removeFromList(child);
                this._appendToList(child);
                // keep children array in sync
            ...
            // Append to linked list and children array
            this._appendToList(child);
            this.children.push(child);
            ...
            child._updateFlags = pixi_js.UPDATE_COLOR | pixi_js.UPDATE_BLEND | pixi_js.UPDATE_VISIBLE | pixi_js.UPDATE_TRANSFORM;
            if (renderGroup) {
                renderGroup.addChild(child);
            }
            const index = this._childCount - 1;
            this.emit('childAdded', child, this, index);
            child.emit('added', this);
            this._didViewChangeTick++;
            return children[0];
        }
        /** Override to remove from linked list */
        ...
        _appendToList(node) {
            if (this._lastChild) {
                this._lastChild.nextChild = node;
                node.prevChild = this._lastChild;
        ...
        _removeFromList(node) {
            const prev = node.prevChild;
            const next = node.nextChild;
            if (prev) {
                prev.nextChild = next;
            }
            else {
            ...
            node.prevChild = node.nextChild = null;
            this._childCount--;
        }
    }
